  ...
  const menuItems = [
    {
      ...
      label: <Link to="/user">Tổng Quan</Link>,
    },
    {
      key: "products",
      icon: <ShoppingOutlined />,
      label: <Link to="/user/products">Sản Phẩm</Link>,
    },
    {
      key: "inventories",
      icon: <DatabaseOutlined />,
      label: <Link to="/user/inventories">Tồn Kho</Link>,
    },
    {
      key: "purchase-orders",
      icon: <ImportOutlined />,
      label: <Link to="/user/purchase-orders">Nhập Hàng (PO)</Link>,
    },
    {
      key: "sales-orders",
      icon: <ExportOutlined />,
      label: <Link to="/user/sales-orders">Xuất Hàng (SO)</Link>,
    },
    {
      key: "shipments",
      icon: <ExportOutlined />,
      label: <Link to="/user/shipments">Giao Hàng</Link>,
    },
    {
      key: "chat",
    ...
    items: [
      {
        ...
        label: (
          <span className="text-gray-500 text-sm">
            Nhân viên: <strong>{user?.username}</strong>
          </span>
        ),
        disabled: true,
      },
      ...
      <Layout className="bg-slate-50">
        <Header className="bg-white px-6 shadow-sm flex items-center justify-between h-16 z-0 relative">
          ...
          <div className="flex items-center gap-3">
            <NotificationBell to="/user/notifications" />
            <Dropdown
              menu={dropdownItems}
