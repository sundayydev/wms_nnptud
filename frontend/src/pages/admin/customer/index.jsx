...
export default function AdminCustomer() {
  ...
  async function fetchData(searchParams) {
    ...
    try {
      setLoading(true);
      let res = await customerService.getAll(searchParams);
      let mappedData = res.map(function (item) {
  ...
  let columns = [
    ...
    },
    {
      ...
      render: function (type) {
        return (
          <Tag
            color={type === "Wholesale" ? "purple" : "geekblue"}
          ...
          <Space size="middle">
            ...
            <Tooltip title="Xoá mềm">
              <Popconfirm
                ...
                description="Khách hàng sẽ được đánh dấu đã xoá và có thể khôi phục theo logic hệ thống."
                okText="Xác nhận"
                cancelText="Hủy"
  ...
  return (
    <div className="animate-fade-in p-6 xl:p-8 bg-gray-50/50 min-h-screen">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-2">
            Quản lý cơ sở dữ liệu khách hàng lẻ và đối tác mua sỉ.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-6 flex items-center justify-between gap-4">
        ...
        />
        <Space>
          <Button onClick={handleSearch} className="h-10 rounded-lg px-6">
            Lọc kết quả
          </Button>
          <Button
            type="primary"
          ...
          locale={{
            emptyText: (
              <CustomEmpty
                ...
                description="Sổ địa chỉ đang trống."
              />
            ),
          }}
      ...
      <Modal
        title={
          <div className="text-2xl font-extrabold text-gray-800 pb-4 border-b border-gray-100 flex items-center gap-2">
            {editingRecord ? (
              <>
                <EditOutlined className="text-indigo-500" /> Sửa Thông Tin Khách
                Hàng
              </>
            ) : (
              <>
                <PlusCircleOutlined className="text-green-500" /> Thêm Khách
                Hàng Mới
              </>
            )}
          </div>
        ...
        okButtonProps={{
          className:
            "bg-indigo-600 hover:bg-indigo-500 shadow-md border-0 h-10 px-6 rounded-lg font-semibold",
        }}
      ...
      >
        <Form
          ...
          layout="vertical"
          className="mt-6"
          initialValues={{ customerType: "Retail" }}
        >
          <div className="grid grid-cols-2 gap-x-8">
            <Form.Item
              name="name"
              label={
                <span className="font-semibold text-gray-700 text-[13px] uppercase tracking-wide">
                  Tên Khách Hàng
                </span>
              }
